document.addEventListener('DOMContentLoaded', () => {
    ...
    // Fills in the top stat cards from the dashboard summary
    ...
    // Renders the critical alerts panel and keeps the nav bell badge count in sync
    ...
    // Builds the open-tickets table rows
    ...
    // Renders a single pole line in the ASCII-tree map, with its live badges
    ...
    // Renders a single home leaf under its parent pole in the tree
    ...
    // Entry point for the live pole map: starts from root poles (no parent) and walks down
    ...
    // Called from the map/search/alerts UI whenever a pole is clicked or looked up
    ...
    // Looks up the typed query as a pole ID first, then a home ID, and selects whichever matches
    ...
    // Marks a ticket resolved on the server, then refreshes the dashboard
    ...
    // Prompts for a new home ID + location and attaches it to the currently selected pole
    ...
    // Pulls the full dashboard snapshot from the server and re-renders everything
    function fetchData() {
